...
class SortablePage(BasePage):
    ...
    def change_list_or_grid_order(self, name_sort):
...
class SelectablePage(BasePage):
    ...
    def select_list_or_grid_item(self, name_select):
        selection = {'select_list': {'tab': self.locators.TAB_LIST,
                                     'items': self.locators.LIST_ITEM,
                                     'active_item': self.locators.LIST_ITEM_ACTIVE},
                     'select_grid': {'tab': self.locators.TAB_GRID,
                                     'items': self.locators.GRID_ITEM,
                                     'active_item': self.locators.GRID_ITEM_ACTIVE}
                     }
        self.element_is_visible(selection[name_select]['tab']).click()
        self.click_selectable_item(selection[name_select]['items'])
        active_element = self.element_is_visible(selection[name_select]['active_item'])
...
class ResizablePage(BasePage):
    ...
    def get_px_from_width_height(self, value_of_size):
        width = value_of_size.split(';')[0].split(': ')[1]
        height = value_of_size.split(';')[1].split(': ')[1]
        return width, height
...
class DraggablePage(BasePage):
    ...
    @allure.step('drag x or y axis')
    def axis_restricted_x_or_y(self, type_axis):
        axis = {'x': self.locators.ONLY_X,
                'y': self.locators.ONLY_Y}
        self.element_is_visible(self.locators.AXIS_TAB).click()
        only_one_axis = self.element_is_visible(axis[type_axis])
        position = self.get_before_and_after_position(only_one_axis)
